...
def createCountry(country: schemas.CountryCreate, db: Session = Depends(getDb)):
    ...
    if dbCountry:
        raise HTTPException(status_code=400, detail="País ya registrado")
...
def readCountries(skip: int = 0, limit: int = 100, db: Session = Depends(getDb)):
    ...
    if countries is None:
        raise HTTPException(status_code=404, detail="No hay países registrados")
    return countries
...
def createCity(city: schemas.CityCreate, db: Session = Depends(getDb)):
    ...
    if dbCity:
        raise HTTPException(status_code=400, detail="Ciudad ya registrada")
